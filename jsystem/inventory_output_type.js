...
$(function () {
    ...
    $('#btn_new_inventory_output_type').click(function () {
        $('#frm_new_inventory_output_type')[0].reset(); //para borrar todos los datos que tenga los input, textareas, select.
        $(".modal-error").empty();
        $('#modal_new_inventory_output_type').modal({
            show: true,
            backdrop: 'static'
        });
    });
    ...
    $('#frm_new_inventory_output_type').submit(function (event) {
        ...
        $.ajax({
            ...
            success: function (respuesta) {
                ...
                if (respuesta.success === true) {
                    ...
                    $('#close_modal_new_inventory_output_type').click();
                    update_data_table($('#list_inventory_output_type'));
    ...
    $('#frm_edit_inventory_output_type').submit(function (event) {
        event.preventDefault();
        var form = $(this);
        ajaxStart('Guardando registros, por favor espere');
        $.ajax({
            ...
            success: function (respuesta) {
                ...
                if (respuesta.success === true) {
                    $('.modal-error').remove();
                    swal('Datos Guardados', 'Los datos se registraron correctamente', 'success');
                    $('#close_modal_edit_inventory_output_type').click();
                    update_data_table($('#list_inventory_output_type'));
                } else if(respuesta.login === true){
                    login_session();
                } else {
...
function edit_register(element) {
	...
	// if (content == 1) {
		$('#edit_id_inventory_output_type').val(id);
		$('#edit_name').val(nombre);
		$('#edit_description').val(descripcion);
		$('#modal_edit_inventory_output_type').modal({
			show: true,
			backdrop: 'static'
		});
	// } else {
...
function delete_register(element) {
	...
	// var content = rowData['contenido'];
	// if (content == 1) {
		delete_register_commom(element, 'inventory_output_type/disable_inventory_output_type');
	// } else {
...
function get_inventory_output_type_list() {
    $('#list_inventory_output_type').DataTable({
        paging: true,
        info: true,
        filter: true,
        stateSave: true,
        ...
        serverSide: true,
        ajax: {
            url: siteurl('inventory_output_type/get_inventory_output_type_list'),
            type: 'post'
        },
        columns: [
            {data: 'id'},
...
function view_register(element) {
    ...
    $('#modal_view_inventory_output_type').modal({
        show: true,
        backdrop: 'static'
    });
}
