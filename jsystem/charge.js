...
$(function () {
    ...
    $('#btn_new_charge').click(function () {
        $('#frm_new_charge')[0].reset(); //para borrar todos los datos que tenga los input, textareas, select.
        $(".modal-error").empty();
        $('#modal_new_charge').modal({
            show: true,
            backdrop: 'static'
        });
    });
    ...
    $('#frm_new_charge').submit(function (event) {
        ...
        $.ajax({
            ...
            success: function (respuesta) {
                ...
                if (respuesta.success === true) {
                    ...
                    $('#close_modal_new_charge').click();

                    update_data_table($('#list_charge'));
    ...
    $('#frm_edit_charge').submit(function (event) {
        event.preventDefault();
        var form = $(this);
        ajaxStart('Registrando datos...');
        $.ajax({
...
function edit_register(element) {
    ...
    var id = rowData['id'];
    var nombre = rowData['descripcion'];
    $('#charge_id').val(id);
    $('#description_edit').val(nombre);
    $('#modal_edit_charge').modal({
        show: true,
        backdrop: 'static'
    });
}
...
function delete_register(element) {
    delete_register_commom(element, 'charge/disable_charge');
}

/*carga la lista de las cargos al datatable*/
function get_charge_list() {
    var tabla = $('#list_charge').DataTable({
        paging: true,
        info: true,
        filter: true,
        stateSave: true,
        ...
        serverSide: true,
        'ajax': {
            "url": siteurl('charge/get_charge_list'),
            "type": 'post'
        },
        'columns': [
            {data: 'id'},
