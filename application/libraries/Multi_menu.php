...
class Multi_menu {
	 ...
	 * @var string
	 */
	private $parent_anchor            = '<a href="%s" class="menu-toggle waves-effect waves-block">%s</a>';
	 ...
	 * @var string
	 */
	private $children_tag_open        = '<ul class="menu-toggle">';
	 ...
	 * @var string
	 */
	private $item_anchor              = '<a href="%s" class="">%s</a>';
	    ...
	    foreach ($items as $item)
	    {
	        ...
	        if ( isset($item[$this->menu_label], $item[$this->menu_key]) ) 
	        {
		        ...
		        if ($icon) 
		        {
		        	...
		        	$label = trim( $this->icon_position == 'right' ? ($label . " " . $icon ) : ($icon . " <span>". $label.'</span>') );
		        }
