 ...
 * Time: 11:52 AM
 */
class Type_inventory_entry_model extends CI_Model
{
    ...
    /*Obtener todas las tipo_ingreso_inventario activas especial para cargar combos o autocompletados*/
    public function get_type_inventory_entry($type = 'object')
    {
        return $this->db->get_where('tipo_ingreso_inventario', array('estado' => get_state_abm('ACTIVO')))->result($type);
    }
    public function get_inventory_by_id($type_inventory_entry_id)
	{
		return $this->db->get_where('tipo_ingreso_inventario', array('id' => $type_inventory_entry_id))->row();
	}
    public function get_type_exit_inventory_enable($type = 'object')
    {
        return $this->db->get_where('tipo_salida_inventario', array('estado' => get_state_abm('ACTIVO'), 'id >' => 3))->result($type);
    }
    public function get_type_exit_inventory_enable1($type = 'object')
    {
        return $this->db->get_where('tipo_salida_inventario', array('estado' => get_state_abm('ACTIVO'), 'id >' => 2))->result($type);
    }
    ...
    public function register_type_warehouse()
    {
        ...
        if ($this->form_validation->run() === TRUE) {
            $data_type_warehouse = array(
                'nombre' => $this->input->post('nombre_tipo_almacen'),
                'descripcion' => $this->input->post('descripcion_tipo_almacen'),
    ...
    public function modify_type_warehouse()
    {
        ...
        if ($this->form_validation->run() === TRUE) {
            $data_type_warehouse = array(
                'nombre' => $this->input->post('nombre_tipo_almacen_edit'),
                'descripcion' => $this->input->post('descripcion_tipo_almacen_edit')
            );
