 ...
 * Time: 15:02 PM
 */
class Inventory_income_type extends CI_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->load->model('inventory_income_type_model');
	}

	public function index()
	{
		template('inventory_income_type/index', array(), 'inventory_income_type');
	}
	...
	public function register_inventory_income_type()
	{
		try {
			if ($this->input->is_ajax_request()) {
				echo json_encode($this->inventory_income_type_model->register_inventory_income_type());
			} else {
				show_404();
			}
		} catch (\Throwable $th) {
	...
	public function modify_inventory_income_type()
	{
		try {
			if ($this->input->is_ajax_request()) {
				echo json_encode($this->inventory_income_type_model->modify_inventory_income_type());
			} else {
				show_404();
			}
		} catch (\Throwable $th) {
	...
	{
		if ($this->input->is_ajax_request()) {
			echo $this->inventory_income_type_model->disable_inventory_income_type();
		} else {
			show_404();
		}
	}
	...
	public function get_inventory_income_type_enable()
	{
		if ($this->input->is_ajax_request()) {
			...
			$inventory_income_type_list = $this->inventory_income_type_model->get_inventory_income_type_enable();
			echo json_encode($inventory_income_type_list);
		} else {
			show_404();
		}
	}
	...
	public function get_inventory_income_type_list()
	{
		if ($this->input->is_ajax_request()) {
			...
			echo json_encode($this->inventory_income_type_model->get_inventory_income_type_list($params));
		} else {
			show_404();
		}
	}
